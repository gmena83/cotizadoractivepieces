...
app.post('/api/cotizador', async (req, res) => {
    ...
    try {
        ...
        // Final email step is now enabled for the final test.
        await sendEmail(clientData.email_cliente, 'Su cotización está lista', proposalHtml, { path: pdfPath, filename: 'Proposal.pdf' });
        ...
        res.sendFile(pdfPath, (err) => {
            if (err) {
                ...
                // If sending the file fails, still send a success response
                if (!res.headersSent) {
                    res.status(500).json({ success: false, error: 'Workflow completed, but failed to send PDF.' });
                }
            } else {
                console.log('Successfully sent PDF to client.');
            }
        });
